...
const BannerWithImageAndText = () => {
	...
	const {
		...
		queryFn: async () => {
			const response = await fetch("https://a12-server-side-livid.vercel.app/bannerData").then(res => res.json());
			return response;
		},
	});
				...
				{bannerData?.map(banner => {
					...
					return (
						<SwiperSlide
							...
							className="px-10 pt-5 sm:pt-0 md:pt-0 my-auto">
							<div className="flex flex-cols md:flex-row justify-between items-center gap-20 relative md:static">
								<img
									src={bannerImage}
									alt="Right Place Image"
									className="w-full md:w-1/2 opacity-30 md:opacity-100"
								/>
								<div className="w-full md:w-1/2 absolute md:relative text-center md:text-left my-auto">
									...
									<img
										width={200}
										className="absolute -top-36 right-0 -z-10 opacity-50 hidden lg:block"
										src={blob02}
										alt=""
									/>
									<img
										width={200}
										className="absolute -top-64 -left-32 -z-10 opacity-50 hidden lg:block"
										src={blob03}
										alt=""
									/>
									<img
										width={200}
										className="absolute -z-10 opacity-50 hidden lg:block"
										src={blob04}
										alt=""
									/>
								</div>
